class UrlsController < ApplicationController
  ...
  def open
    ...
    GoogleAnalyticsApi.new.event('gifts', 'url opened', UrlParser.domain(url.src), 555, user_type: current_user.role, list_type: url.gift.list.occasion)
    redirect_to url.src
  end

  def create
    begin
      @data = LinkThumbnailer.generate(UrlParser.match(url_params[:data]))
    ...
    @url.digest =  Digest::SHA1.hexdigest(UrlParser.match(url_params[:data]))
